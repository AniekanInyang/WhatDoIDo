      ...
      <article className="surface-card p-4">
        <div className="grid gap-2">
          {historyItems.map((item) => (
            <div
              key={item.id}
              className="flex flex-col gap-2 rounded-xl border border-brand-border bg-brand-soft p-3 md:flex-row md:items-center md:justify-between"
            >
              <div>
              ...
              <div className="flex items-center gap-2 text-xs text-brand-muted">
                <span>{item.date}</span>
                <span className="rounded-full bg-brand-primary/20 px-2.5 py-1 text-brand-text">{item.confidence}</span>
              </div>
            </div>
          ))}
        </div>
